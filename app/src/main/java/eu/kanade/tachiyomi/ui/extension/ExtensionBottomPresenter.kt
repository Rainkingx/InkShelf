...
class ExtensionBottomPresenter : BaseMigrationPresenter<ExtensionBottomSheet>() {
            ...
            available
                ...
                .filter { avail ->
                    installed.none { it.pkgName == avail.pkgName } &&
                        ...
                        // "all" is the repository-wide extension group used by the original
                        // Extensions screen. Always keep it visible, even if the language
                        // filter preference was changed elsewhere.
                        (avail.lang == "all" || avail.lang in activeLangs) &&
    ...
    fun findAvailableExtensions() {
        presenterScope.launch {
            // Refresh the repository and immediately redraw the list. Previously this
            // fetched the remote index without rebuilding the adapter, which could leave
            // the available/"All" section missing until the screen was recreated.
            extensionManager.findAvailableExtensions()
            extensions =
                toItems(
                    Triple(
