...
open class GlobalSearchController(
        ...
        searchView?.setOnQueryTextListener(
            object : SearchView.OnQueryTextListener {
                override fun onQueryTextSubmit(query: String?): Boolean {
                    if (!restoringSearch && isControllerVisible) {
                ...
                override fun onQueryTextChange(newText: String?): Boolean {
                    if (!restoringSearch &&
                        ...
                        router.backstack.lastOrNull()?.controller == this@GlobalSearchController
                    ) {
                        presenter.search("")
                        setTitle()
                    }
    ...
    override fun onActionViewExpand(item: MenuItem?) {
        val wasRestoring = restoringSearch
        restoringSearch = true
        try {
            activityBinding?.searchToolbar?.searchView?.setQuery(presenter.query, false)
        } finally {
            restoringSearch = wasRestoring
        }
    }
    ...
    override fun onViewCreated(view: View) {
        ...
        // The presenter survives comic navigation; restore results after creating the adapter.
        adapter?.updateDataSet(presenter.items)
