...
class LibraryGridHolder(
    ...
    init {
        ...
        if (fixedSize) {
            ...
            // Uniform grid: use a conventional comic-book portrait ratio.
            // Keeping every cover at the same 15:22 geometry prevents uneven rows,
            // while CENTER_CROP fills the card cleanly without letterboxing.
        ...
        if (compact) {
            ...
            binding.compactTextLayout.isVisible = true
        } else {
            binding.compactTextLayout.isVisible = false
    ...
    override fun onSetValues(item: LibraryItem) {
        ...
        when {
            compact -> {
                binding.compactTitle.text = item.manga.title.highlightText(filter, color)
                val total = item.manga.totalChapters
                binding.compactSubtitle.text =
                    if (item.manga.unread > 0) {
                        view.context.getString(
                            R.string.modern_library_chapters_unread,
                            total,
                            item.manga.unread,
                        )
                    } else {
                        view.context.getString(R.string.modern_library_chapters, total)
                    }
            }
