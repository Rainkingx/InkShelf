...
class StatsController : BaseController<StatsControllerBinding>() {
     ...
     */
    override fun getTitle() = resources?.getString(R.string.inkshelf_reading_universe)
    ...
    private fun handleMonthlyStats() {
        val currentMonth = presenter.getMonthlyReadingStats()
        val previousMonth = presenter.getMonthlyReadingStats(-1)
        val difference = currentMonth.chaptersRead - previousMonth.chaptersRead

        with(binding) {
            statsMonthChaptersText.text = currentMonth.chaptersRead.toString()
            statsMonthTitlesText.text = currentMonth.titlesTouched.toString()
            statsMonthDurationText.text = currentMonth.readDuration
            statsMonthComparisonText.text =
                when {
                    difference > 0 -> activity?.getString(R.string.inkshelf_ru_month_more, difference)
                    difference < 0 -> activity?.getString(R.string.inkshelf_ru_month_fewer, -difference)
                    else -> activity?.getString(R.string.inkshelf_ru_month_same)
                }
        }
    }

    private fun handleTopWorlds() {
        val topWorlds = presenter.getTopWorldsStats()
        val none = activity?.getString(R.string.none).orEmpty()

        with(binding) {
            statsTopSeriesText.text = topWorlds.topSeries ?: none
            statsTopSeriesDetail.text =
                activity?.getString(R.string.inkshelf_ru_read_count, topWorlds.topSeriesReads)

            statsTopGenreText.text = topWorlds.topGenre ?: none
            statsTopGenreDetail.text =
                activity?.getString(R.string.inkshelf_ru_read_count, topWorlds.topGenreReads)

            statsTopCreatorText.text = topWorlds.topCreator ?: none
            statsTopCreatorDetail.text =
                activity?.getString(R.string.inkshelf_ru_read_count, topWorlds.topCreatorReads)

            statsLongestSeriesText.text = topWorlds.longestSeries ?: none
            statsLongestSeriesDetail.text =
                activity?.getString(R.string.inkshelf_ru_chapter_count, topWorlds.longestSeriesChapters)
        }
    }
