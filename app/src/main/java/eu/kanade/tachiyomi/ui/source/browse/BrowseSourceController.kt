...
open class BrowseSourceController(
    ...
    // Some browse actions intentionally collapse the source search field.
    // Do not treat those internal collapses as navigation back to global search.
    private var suppressSearchCollapseReset = false
...
//    }
    ...
    override fun onActionViewCollapse(item: MenuItem?) {
        if (suppressSearchCollapseReset) {
            suppressSearchCollapseReset = false
            return
        }

        // A filter sheet can temporarily affect focus/search UI state.
        // Keep the current source query and controller in place while it is open.
        if (filterSheet != null) return

        // When this source page was opened from grouped global-search results,
        // collapsing the search is the user's Back action. Return immediately to
        // the grouped carousels instead of clearing the query and starting another
        // network request first, which made Back feel stuck.
    ...
    private fun resetPager() {
        ...
        if (searchItem?.isActionViewExpanded == true) {
            suppressSearchCollapseReset = true
            val collapsed = searchItem.collapseActionView()
            if (!collapsed) {
                suppressSearchCollapseReset = false
            }
        }
