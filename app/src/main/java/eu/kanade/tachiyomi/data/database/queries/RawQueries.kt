...
/**
 * Returns the latest history row for only the supplied manga IDs.
 * The modern Library uses this instead of paging through the user's entire reading history.
 */
fun getRecentMangasForIdsQuery(mangaIds: Collection<Long>): String {
    require(mangaIds.isNotEmpty())
    val ids = mangaIds.joinToString(",")
    return """
        SELECT ${Manga.TABLE}.${Manga.COL_URL} as mangaUrl, ${Manga.TABLE}.*, ${Chapter.TABLE}.*, ${History.TABLE}.*
        FROM ${Manga.TABLE}
        JOIN ${Chapter.TABLE}
        ...
        ON ${Chapter.TABLE}.${Chapter.COL_ID} = ${History.TABLE}.${History.COL_CHAPTER_ID}
        JOIN (
            SELECT ${Chapter.TABLE}.${Chapter.COL_MANGA_ID},
                   ${Chapter.TABLE}.${Chapter.COL_ID} as ${History.COL_CHAPTER_ID},
                   MAX(${History.TABLE}.${History.COL_LAST_READ}) as ${History.COL_LAST_READ}
            FROM ${Chapter.TABLE}
            JOIN ${History.TABLE}
            ON ${Chapter.TABLE}.${Chapter.COL_ID} = ${History.TABLE}.${History.COL_CHAPTER_ID}
            WHERE ${Chapter.TABLE}.${Chapter.COL_MANGA_ID} IN ($ids)
            GROUP BY ${Chapter.TABLE}.${Chapter.COL_MANGA_ID}
        ) AS max_last_read
        ON ${Chapter.TABLE}.${Chapter.COL_MANGA_ID} = max_last_read.${Chapter.COL_MANGA_ID}
        AND max_last_read.${History.COL_CHAPTER_ID} = ${History.TABLE}.${History.COL_CHAPTER_ID}
